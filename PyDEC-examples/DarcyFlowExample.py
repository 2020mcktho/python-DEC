...
# Note, d1.T gives the transpose of the matrix d1
...
# calculate flux values at the boundary edges
for i, e in enumerate(boundary_edges):
    evector = (-1)**e.parity * (vertices[e[1]] - vertices[e[0]])  # vector along the boundary edge
    normal = array([-evector[1], evector[0]])  # normal to the edge vector
    all_fluxes[boundary_indices[i]] = -s * (1/norm(evector)**2) * \
                              dot(velocity, normal) * \
                              abs(det(vstack((normal, evector))))
pressures = zeros(N2)
# Adjust RHS for known fluxes and pressures
# Note: all_fluxes has size N1, pressures has size N2, so concatenating them gives size N1+N2
